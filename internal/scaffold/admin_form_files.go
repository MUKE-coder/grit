...
// adminMoneyField returns the amount-plus-currency field component.
//
// Separate from NumberField because the two halves have to move together:
// the number of decimals shown, and what a typed figure means, both depend
// on the currency selected next to it.
func adminMoneyField() string {
	return `import { useEffect, useId, useState } from "react";
import { Input } from "@/components/ui/input";
import type { FieldDefinition } from "@/lib/resource";
