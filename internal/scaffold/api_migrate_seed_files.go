...
func writeMigrateSeedFiles(root string, opts Options) error {
	...
	files := map[string]string{
		...
		filepath.Join(apiRoot, "internal", "database", "seedbatch.go"):    apiSeedBatchGo(),
...
//
// It does three jobs: migrate, show what previous runs changed (--status), and
// undo a run (--down). The last one exists because AutoMigrate has no down
// scripts, so the only honest way to offer a rollback is to record what each run
// actually added and drop that. internal/migrate does the recording; this file
// is the command around it, including the part that refuses to drop columns
...
// apiSeedBatchGo returns internal/database/seedbatch.go: the batched, resumable
// seeding every generated --faker seeder calls, and the registry behind
// "grit seed <Resource> --count N". Framework-owned, so upgrade delivers it.
func apiSeedBatchGo() string {
...
` + seedDemoGuardNew + `	users := []models.User{
