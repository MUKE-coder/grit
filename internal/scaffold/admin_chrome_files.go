...
// adminSessionWatchdogComponent — silent session manager that warns the
// user before their JWT expires and gives them a chance to extend.
//
// Why: HttpOnly cookie JWTs expire silently. Without a warning the user
// loses unsaved form work the moment they click "save" 16 minutes after
// they last loaded the page. This component:
//   - Tracks last user activity (mouse / keyboard / touch).
//   - At IDLE_WARN_MS of inactivity (default 14:30), opens a modal with
//     a 30-second countdown asking "Stay signed in or sign out?".
//   - "Stay signed in" POSTs /api/auth/refresh (the API rotates the
