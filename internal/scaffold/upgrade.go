...
func Upgrade(uOpts UpgradeOptions) error {
	...
	if hasAPI {
		...
		// Generated CSV imports: each belongs_to value looked up once, lookup
		// errors fail the row, imports take turns, and the WAF no longer
		// refuses a file over 1 MB. After the codegen runtime, which delivers
		// internal/imports.
		...
		// The admin's stat cards read their counts from the list request, so the
		// users and blog list handlers return them.
		...
		// Build contexts, the production stack, SQLite in production and the admin
		// panel's sign-in gate.
		...
		// Activity rows written in batches, scheduled jobs with retry limits, an
		// activity-log prune that keeps the chain, and a shutdown that stops the
		// outbox relay.
