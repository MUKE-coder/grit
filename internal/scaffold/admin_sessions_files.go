...
// Active-sessions UI.
//
// The API side (internal/models/session.go, services/session.go,
// handlers/session_handler.go) records every issued refresh token as a row so it
// can be revoked. This is the screen that makes that visible to the person whose
// account it is: every signed-in device, when it was last used, and a button to
// kill it.
//
// It lives on the profile page directly under "Password", because the two things
// a user does after suspecting a compromise are change the password and sign the
// other devices out — and doing the first now automatically does the second.

func adminActiveSessions() string {
	return `"use client";
