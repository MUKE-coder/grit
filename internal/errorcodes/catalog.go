...
var catalog = []Entry{
	...
	{"INVALID_CREDENTIALS", http.StatusUnauthorized, CategoryAuth, "auth",
		"The email and password do not match an account. A locked account, and one that signs in only with a social provider, get this too until the password is right.",
		"Say only that the details are wrong, and offer password reset: which of the two it was, and whether the account exists or is locked, is deliberately not reported."},
	...
	{"ACCOUNT_LOCKED", http.StatusTooManyRequests, CategoryLimit, "auth",
		"Too many wrong two-factor codes, so the account is locked for a while. Password sign-in reports a lock as INVALID_CREDENTIALS.",
	...
	{"SOCIAL_AUTH_ONLY", http.StatusBadRequest, CategoryRequest, "auth",
		"The account signs in with a social provider and has no password. Only projects from before sign-in stopped reporting it return this; they now get INVALID_CREDENTIALS.",
	...
	{"UPLOAD_KEY_FORBIDDEN", http.StatusForbidden, CategoryPermission, "uploads",
		"The key being recorded was not presigned for this user. Only keys under the caller's own uploads/<user_id>/ prefix are accepted.",
		"Send back the key the presign returned. Another user's key, or one outside the uploads, will not be accepted."},
	{"UPLOAD_ALREADY_RECORDED", http.StatusConflict, CategoryConflict, "uploads",
		"This upload has already been recorded, and a key is recorded only once.",
		"Treat the upload as done: fetch it from the uploads list instead of completing it again."},
