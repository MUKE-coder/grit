...
> **Background:** a bold gradient from indigo (#6366F1) to deep navy (#0B1120),
> overlaid with a subtle evenly-spaced dot-grid pattern. **Headline:** the words
> **"Full-stack CRUD\nin one command"** in a heavy geometric sans-serif, pure
> white, inside a solid deep-navy highlight block anchored lower-left. **Right
> third:** two overlapping glossy code windows fanned out — the front one titled
> `product.go` showing a few lines of Go, the back one titled `use-products.ts`
> showing TypeScript — a thin glowing connector line between them implying
> "generated together," soft indigo glow. **Top-left:** place the attached Grit
