/*
 * Three steps across, with arrows between them.
 *
 * The arrows are `aria-hidden` and disappear below `md`, where the steps
 * stack. An arrow pointing right at a step that is now underneath is worse
 * than no arrow: it tells the eye to look somewhere there is nothing.
 *
 * It is an <ol>, because the sequence carries the meaning. A screen reader
 * announcing "3 of 3" is the audible version of the arrows.
 */
...
const STEPS = [
  ...
  },
  {
    ...
    body: 'The pipeline processes complex datasets and surfaces the patterns in them.',
    Glyph: CurrencyGlyph,
  },
  {
    title: 'Actionable reports',
    body: 'Turn the result into visualisations and reports you can hand to anyone.',
    Glyph: ReportsGlyph,
  },
]
...
function Arrow() {
  return (
    <svg
      ...
      viewBox="0 0 24 24"
      fill="none"
      className="size-6 text-gray-300 dark:text-gray-700"
    >
      <path
...
export default function HowItWorksThreeStepsWithArrows({
  eyebrow = 'Our process',
  ...
  return (
    <section className="bg-white py-24 sm:py-32 dark:bg-gray-950">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <ol
          role="list"
          className="mx-auto mt-20 grid max-w-5xl grid-cols-1 items-start gap-x-6 gap-y-12 md:grid-cols-3"
        >
          {steps.map((step, i) => (
            <li key={step.title} className="relative">
              <div className="text-center">
                ...
                <p className="mx-auto mt-2 max-w-xs text-sm/6 text-gray-600 dark:text-gray-400">
                  {step.body}
                </p>
              </div>
              {/* Sits in the gutter between two columns, and only exists once
                  there is a gutter. Below md the steps stack and an arrow
                  pointing sideways at a step that is now underneath points at
                  nothing. */}
              {i < steps.length - 1 && (
                <div className="absolute top-24 -right-3 hidden translate-x-1/2 md:block">
                  <Arrow />
                </div>
              )}
            </li>
