...
export const CLI_COMMANDS: CliCommand[] = [
  ...
  // ── Meta ───────────────────────────────────────────────────────────
  {
    ...
    example: 'grit upgrade --diff',
    output: [
      '  Comparing against v3.168.0...',
      '',
      '  ~ apps/api/internal/paginate/paginate.go   (2 additions)',
      '  ~ apps/admin/components/ui/button.tsx      (1 addition)',
      '  = 412 files unchanged',
      '  ! apps/api/internal/handlers/product.go    edited by you, left alone',
      '',
      '  Run without --diff to apply.',
    ],
    purpose:
      'Updates the framework files inside an existing project. It reads .grit/manifest.json to know which generator wrote which file at which version and with what content hash, so a file you edited is left alone rather than overwritten.',
    useCases: [
      'After `grit update` brings a new CLI.',
      'Picking up a bug fix in a scaffolded file.',
      '--diff first, always, to see what would change.',
    ],
    files: [
      { path: 'framework files under apps/', status: 'modified', note: 'only the untouched ones' },
      { path: '.grit/manifest.json', status: 'modified' },
    ],
    flags: [
      { flag: '--diff', desc: 'Show what would change in the files it would leave alone' },
      { flag: '--force', desc: 'Overwrite unconditionally, the pre-manifest behaviour' },
    ],
    notes: ['Commit before you run it without --diff.'],
    docs: [{ label: 'Versioning', href: '/docs/versioning' }],
    keywords: ['update project', 'framework update', 'migrate version'],
  },
  {
    id: 'update',
    name: 'grit update',
