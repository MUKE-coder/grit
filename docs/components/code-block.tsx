...
export function CodeBlock({
  ...
  return (
    <div className={`rounded-lg border border-white/[0.06] bg-[#0d1117] overflow-hidden relative group ${className}`}>
      {/* Header */}
      {(filename || terminal) && (
    ...
    <div className="relative grid grid-cols-1 lg:grid-cols-[minmax(min-content,350px)_minmax(min-content,1fr)] gap-x-10 gap-y-4 pb-16">
      <div>
        <div className="flex items-center gap-3 mb-3">
          <span className="inline-flex items-center justify-center h-7 px-2 rounded border border-white/10 bg-white/5 text-[11px] font-mono font-medium text-slate-400">
            {number}
          </span>
