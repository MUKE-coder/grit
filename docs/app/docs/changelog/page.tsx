      ...
      - action: cancel
        ...
        confirm: true`} />
            ...
            <div className="mb-12">
              ...
              <div className="prose-grit">
                <p>
                  <strong>grit upgrade stops overwriting the files you have edited.</strong>
                </p>
                <p>
                  It used to overwrite all of them, every time. The function that wrote
                  them took a <code>force</code> parameter and never read it, so the flag
                  was decoration and every upgrade was a forced one. If you had changed a
                  framework component, the upgrade took it back and told you it had updated
                  87 files.
                </p>
                <p>
                  Grit now records what it writes. Every generated file gets an entry in{' '}
                  <code>.grit/manifest.json</code>: which generator wrote it,
                  at which version, and the hash of what it wrote. An upgrade compares that
                  against what is on disk, so it can tell a file nobody has opened from one
                  you have customised. Untouched files are replaced. Edited ones are named
                  and left exactly as they are.
                </p>
                <p>
                  <code>grit upgrade --diff</code> prints a unified diff of your version
                  against the new one, so you can take the parts you want by hand.{' '}
                  <code>grit upgrade --force</code> does what upgrade always did.
                </p>
                <p>
                  The check sits at the single function every generated byte passes
                  through, not at the callers. Upgrade regenerates the web app, the admin,
                  the docs and the root config through four different paths that fan out to
                  dozens of template functions, and one check at the one choke point is
                  both smaller and harder to leave a hole in.
                </p>
                <p>
                  Two details that are not cosmetic. Hashes are taken with line endings
                  normalised, because git on Windows checks files out with CRLF while the
                  generator writes LF, and hashing raw bytes would report every file in a
                  fresh clone as edited: an upgrade trusting that would refuse to update
                  anything. And injection re-records rather than invalidates, so adding a
                  route to <code>routes.go</code> does not make it read as hand-edited from
                  then on.
                </p>
                <p>
                  A project created before this release has no manifest, so nothing can be
                  said about what has been edited in it, and its first upgrade behaves
                  exactly as it always did. That upgrade writes the manifest. Every one
                  after it is protected. Commit{' '}
                  <code>.grit/manifest.json</code> so the whole team gets it.
                </p>
                <p>
                  Also: <code>grit generate resource</code> records its files under the
                  resource that owns them, which is what{' '}
                  <code>grit upgrade --resource</code> will read next.
                </p>
              </div>
            </div>
