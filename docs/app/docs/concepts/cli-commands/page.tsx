...
export default function CLICommandsPage() {
        ...
        <div className="max-w-4xl mx-auto py-12 px-6 lg:px-8">
          ...
          <div className="mb-14">
            <h2 className="text-xl font-semibold text-white mb-4">
              <code className="text-sky-400 font-mono">grit routes</code>
            </h2>
            <p className="text-slate-400 mb-6">
              List all registered API routes in a formatted table. Parses your{' '}
              <code className="text-sky-400 bg-white/[0.04] px-1.5 py-0.5 rounded text-[13px]">routes.go</code> file
  ...
  16 routes total`} />
            <p className="text-sm text-slate-500 mt-4">
              Works for both monorepo (<code className="text-slate-400">apps/api/internal/routes/</code>) and
              single app (<code className="text-slate-400">internal/routes/</code>) projects.
            </p>
          </div>
          ...
          <div className="mb-14">
            <h2 className="text-xl font-semibold text-white mb-4">
              <code className="text-sky-400 font-mono">grit down</code> / <code className="text-sky-400 font-mono">grit up</code>
            </h2>
            <p className="text-slate-400 mb-6">
              Toggle maintenance mode. When enabled, all API requests receive a{' '}
              <code className="text-sky-400 bg-white/[0.04] px-1.5 py-0.5 rounded text-[13px]">503 Service Unavailable</code> response.
            </p>
  ...
  Normal request handling has resumed.`} className="" />
            ...
            <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-5">
              ...
              <ul className="space-y-2 text-sm text-slate-400">
                <li className="flex items-start gap-2">
                  ...
                  <code className="text-slate-300">grit down</code> creates a <code className="text-slate-300">.maintenance</code> file in the project root
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-sky-400 mt-0.5">2.</span>
                  The scaffolded <code className="text-slate-300">Maintenance()</code> middleware checks for this file on every request
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-sky-400 mt-0.5">3.</span>
                  <code className="text-slate-300">grit up</code> removes the file, resuming normal operation
                </li>
              </ul>
            </div>
    ...
    return func(c *gin.Context) {
        ...
        c.Next()
          ...
          <div className="mb-14">
            <h2 className="text-xl font-semibold text-white mb-4">
              <code className="text-sky-400 font-mono">grit deploy</code>
            </h2>
            <p className="text-slate-400 mb-4">
              One-command production deployment. See the dedicated{' '}
              <Link href="/docs/infrastructure/deploy-command" className="text-sky-400 hover:underline">
                Deploy Command
              </Link>{' '}
...
grit deploy`} />
          ...
          <div className="mb-14">
            <h2 className="text-xl font-semibold text-white mb-4">Complete command reference</h2>
            <div className="overflow-x-auto rounded-lg border border-white/[0.06]">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.06] bg-white/[0.02]">
                    <th className="px-4 py-3 text-left font-medium text-slate-300">Command</th>
                ...
                <tbody className="divide-y divide-white/[0.04]">
                  {[
                    ...
                    <tr key={cmd} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-4 py-2.5 font-mono text-[13px] text-sky-400 whitespace-nowrap">{cmd}</td>
