...
subtitle: "You bought a dashboard template and you want its pages in your admin — without giving up URL-synced sorting, paging, filters, selection, bulk delete, toasts and cache invalidation. Grit's customisation system in six levels, from patching one cell to replacing an entire page, with a real app built at each step."
series: "The Daily Grit"
edition: 15
date: 2026-08-12
readingTime: "13 min"
author: "Muke JohnBaptist"
tags: [grit, admin, customisation, templates, react, typescript, hooks]
...
stop climbing the moment you have what you need.
...
The cheapest thing you can do, and the one that covers most complaints. The
table is fine; three cells are not. A raw float is not a price, a bare word is
not a status, and a number on its own says nothing about whether you are about
to run out of stock.
...
**Nothing else changed.** Sorting still works on those columns. Search still
works. The exporter still exports. You replaced three functions, not a page.
...
Same idea, other half of the resource. The form fields come from your Go struct;
the things a struct cannot express — a placeholder, a helper line, a minimum —
go here:
...
Merged by key, same as columns. The label, the type and the required flag still
come from the generator.

## Level 3: replace the table

Now we are into template territory. Your bought component draws a list, not a
grid, and you want *that* one:
...
A form slot owns its own saving, which sounds like a lot and is four lines,
because the hooks the stock form uses are exported:
...
function Composer({ resource, item, onClose }: ResourceFormProps<Enquiry>) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (item) update.mutate({ id: item.id, body: form }, done);
    else create.mutate(form, done);
  };

  return <form onSubmit={submit}>{/* your markup */}</form>;
}
```

`item` is `null` for create and the record for edit — that one prop is the whole
mode switch. The mutations invalidate the query and raise the toast for you, so
the list behind the dialog updates itself the moment you close.

Your form replaces whichever container `formView` would have opened — sheet,
modal, page. If you want it in a drawer, put it in a drawer; the slot does not
care.

## Level 5: replace the empty state

The smallest slot, and the one people forget until a client sees a blank table
on day one:
...
The template had a kanban board and no table at all. Fine:
...
Why trust that the hook is complete? Because the stock page is built on it and
contains no state of its own — it is markup and nothing else. If the controller
were missing something, the default page could not exist. So anything the default
page can do, yours can too. That is not a promise, it is the file layout.
...
Leaving it would break the build — it imports a type the shared package no longer
exports. Deleting it outright would throw away work the generator never owned.
So it goes sideways.
...
2. **Move the shell into `globals.css`.** Grit's colour tokens are CSS variables
...
5. **Replace the page only when there is no table.** Kanban, split-pane inbox,
   canvas. If you find yourself rebuilding pagination inside a Page slot, you
   wanted Level 3.
6. **Non-CRUD screens skip all of it** and call `useResource` directly.
